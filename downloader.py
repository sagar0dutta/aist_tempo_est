...
SOURCE_URL = 'https://aistdancedb.ongaaccel.jp/v1.0.0/video/2M/'
LIST_URL = 'https://storage.googleapis.com/aist_plusplus_public/20121228/video_list.txt'

def _download(video_url, download_folder, retries=3, delay=5):
  save_path = os.path.join(download_folder, os.path.basename(video_url))
  # urllib.request.urlretrieve(video_url, save_path)

  if os.path.exists(save_path):
        print(f"Skipping {os.path.basename(video_url)} (Already Downloaded)")
        return

  for attempt in range(1, retries + 1):
    try:
        # print(f"Downloading {video_url} (Attempt {attempt}/{retries})...")
        urllib.request.urlretrieve(video_url, save_path)
        print(f"\nSuccessfully downloaded in Attempt {attempt}: {save_path}")
        return  # Exit after a successful download
    except urllib.error.ContentTooShortError as e:
        print(f"ContentTooShortError: {e}. Retrying in {delay} seconds...")
    ...
    print(f"\nFailed to download {video_url} after {retries} attempts.")
...
if __name__ == '__main__':
  ...
  download_func = partial(_download, download_folder=args.download_folder, retries=3, delay=5)
