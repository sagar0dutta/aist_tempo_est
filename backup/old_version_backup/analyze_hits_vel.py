...
class HitsAnalysis:
    ...
    def load_files(self):
        ...
        self.foot_1S_df = pd.read_csv(f"./aist_vel1s/{self.norm_mode}/tempo_{self.a}_{self.b}/foot/{self.foot1s_name}_zero_{self.mode}_{self.a}_{self.b}.csv")
        foot_2S_df = pd.read_csv(f"./aist_vel2s/{self.norm_mode}/tempo_{self.a}_{self.b}/foot/foot_zero_{self.mode}_{self.a}_{self.b}.csv")

        foot_1s_score = pd.read_csv(f"./aist_vel1s/{self.norm_mode}/tempo_{self.a}_{self.b}/score/foot/{self.foot1s_name}_{self.mode}_{self.a}_{self.b}_score.csv")
        hand_1s_score = pd.read_csv(f"./aist_vel1s/{self.norm_mode}/tempo_{self.a}_{self.b}/score/hand/{self.hand1s_name}_{self.mode}_{self.a}_{self.b}_score.csv")

        foot_2s_score = pd.read_csv(f"./aist_vel2s/{self.norm_mode}/tempo_{self.a}_{self.b}/score/foot/foot_{self.mode}_{self.a}_{self.b}_score.csv")
        hand_2s_score = pd.read_csv(f"./aist_vel2s/{self.norm_mode}/tempo_{self.a}_{self.b}/score/hand/hand_{self.mode}_{self.a}_{self.b}_score.csv")
    ...
    def calc_totalhits(self, hits_1s, hits_2s):
        ...
        hit_total.to_csv(f"./stats_vel/{self.norm_mode}/tempo_{self.a}_{self.b}/{self.mode}/hits_total/{self.hand1s_name}_{self.foot1s_name}.csv", index=False)
    ...
    def calc_genrewise_hits(self, hits_1s, hits_2s):
        ...
        config_hits = {
            "combined_1S_2S": {
                ...
                "para": ["combined_hits_1S", "combined_hits_2S", f"./stats_vel/{self.norm_mode}/tempo_{self.a}_{self.b}/{self.mode}/genre_wise/combined_{self.hand1s_name}_{self.foot1s_name}_genrewise_hits"]
            },
            "hand_1S": {
                "L1": [hits_1s["hand_hits"], hits_1s["foot_hits"]],
                "para": ["hand_hits_1S", "foot_hits_1S", f"./stats_vel/{self.norm_mode}/tempo_{self.a}_{self.b}/{self.mode}/genre_wise/{self.hand1s_name}_{self.foot1s_name}_1S_genrewise_hits"]
            },
            "hand_2S": {
                "L1": [hits_2s["hand_hits"], hits_2s["foot_hits"]],
                "para": ["hand_hits_2S", "foot_hits_2S", f"./stats_vel/{self.norm_mode}/tempo_{self.a}_{self.b}/{self.mode}/genre_wise/both_hand_foot_2S_genrewise_hits"]
            }
        }
